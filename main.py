...
def mainGSS(htmlinput, cssinput):
    ...
    def selectortolinenumber(selector):
        if "::" in selector or ":root" in selector:
            return 0
    ...
    css_no_at, removed_statements, removed_blocks = format_css2(css_data)

    splits = css_no_at.split("}")
    css_nice = []

    with open("/home/gijs3/mysite/overig/order.json") as orderfile:
